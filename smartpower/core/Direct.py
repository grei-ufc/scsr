...
def carregar_dados(self):
	...
	for item in self.scene.items():
		if isinstance(item, Node):
			...
			condutor['ampacidade']=float(sitem.linha.ampacidade)
			condutor['n1_id']=float(item.w1.id)
			condutor['n2_id']=float(item.w2.id)
